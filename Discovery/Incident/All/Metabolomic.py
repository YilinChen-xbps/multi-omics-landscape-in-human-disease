...
tgt_done_lst = glob.glob(dpath + 'Results/Associations/Step1/'+group+'/'+subgroup+'/'+omics+'/*.csv')
tgt_done_lst = [os.path.basename(ele)[:-4] for ele in tgt_done_lst]
tgt_lst = [ele for ele in tgt_lst if ele not in tgt_done_lst]

omics_df = pd.read_csv(dpath + 'Data/BloodData/MetabolomicData.csv')
...
for tgt in tqdm(tgt_lst):
    ...
    try:
        ...
        #tgt_out_lst.append([process(omics_f, omics_df, tgt_df, my_formula) for omics_f for f in omics_f_lst])
        tgt_out_df = pd.DataFrame(tgt_out_lst)
