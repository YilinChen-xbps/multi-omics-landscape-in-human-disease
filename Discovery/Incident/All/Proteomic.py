...
omics_df = pd.read_csv(dpath + 'Data/BloodData/ProteinData_n_Cov.csv')
omics_f_lst = omics_df.columns[13:2933].tolist()
omics_df = omics_df[['eid']+omics_f_lst+[omics_f + '_SampAge' for omics_f in omics_f_lst]]
...
cov_f_lst = ['eid', 'Age', 'Sex', 'Race', 'TDI_imp', 'BMI_imp', 'Smoke_imp', 'FastingTime_imp', 'Season']
formula_full = "Age + C(Sex) + Age*Age + Age*C(Sex) + Age*Age*C(Sex) + TDI_imp + C(Smoke_imp) + BMI_imp + FastingTime_imp + C(Season) + x_omics_sa + x_omics"
formula_non_sex = "Age + Age*Age + TDI_imp + C(Smoke_imp) + BMI_imp + FastingTime_imp + C(Season) + x_omics_sa + x_omics"
